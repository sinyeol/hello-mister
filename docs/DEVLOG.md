...
## Arcade Database, New Platform Discovery, And v1 Archive (2026-09-15)

- The old standalone v1 app folder (`zaparoo-nfc-card-stickers`) is archived and removed. Its behavioural tests are ported into v2 as `scripts/test-sticker-*.mjs` (sheet editor / print pages, pagination and print pipeline contracts, asset source groups, card album, platform identity, arcade database) and run in `npm test`.
- `package.json` scripts, electron-builder config and devDependencies were restored after the v2.1.1 release commit had committed the trimmed asar-style file.
- Platform identity keys no longer include MiSTer menu-root folder names (`_Arcade`, `_Console`, `_Computer`, `games`). Cores installed flat under `_Arcade` and split by their `<rbf>` (for example jtcps3) previously matched the generic `Arcade` platform and could not be imported through 새 플랫폼 발견 or 가져올 플랫폼; they now show up as new platforms.
- The 새 플랫폼 발견 시 scan-filter setting is applied automatically after a scan: 가져오기로 추가 merges newly discovered platforms right away, 제외로 추가 registers them as excluded, 숨김 hides them, 확인 후 선택 keeps the manual list.
- Arcade games are named and described from the MiSTer Arcade Database (`mad_db.json`) that the update_all Arcade Organizer keeps as `Scripts/.config/arcade-organizer/data.zip`: `list-arcade-cores` reads `<rbf>` and `<setname>` per `.mra`, the new read-only IPC `mister:remote:read-arcade-database` unzips the database over SFTP and caches it in userData, cores missing from the curated table take the majority database platform name, and every arcade entry stores genre, year, manufacturer, orientation and an arcade metadata block (players, buttons, rotation, alternative/bootleg/homebrew, series). `_Arcade Organized` itself (a symlink view) is never scanned.
- Library merge follows a rescan's platform name only when the stored name is the raw core name or the generic Arcade bucket, so custom platform names are never overwritten.
- The unreachable connection form on the sticker MiSTer page and the leftover HTTP bridge calls in Electron were removed.
